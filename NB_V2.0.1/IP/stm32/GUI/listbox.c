					...
					}else if(listbox->sta&(1<<6))//单点按下
					{
						...
						{		 
							if((listbox->selindex<(listbox->scbv->topitem+listbox->scbv->itemsperpage))&&(listbox->selindex>=listbox->scbv->topitem))//老的selindex在屏幕范围内
							{
								...
								gui_show_ptstr(listbox->left,listbox->top+i*LBOX_ITEM_HEIGHT+(LBOX_ITEM_HEIGHT-listbox->font)/2,endx,GUI_LCD_H,0,listbox->lncolor,listbox->font,listx->name,1);//显示名字
							}
							...
							gui_show_ptstr(listbox->left,listbox->top+tempindex*LBOX_ITEM_HEIGHT+(LBOX_ITEM_HEIGHT-listbox->font)/2,endx,GUI_LCD_H,0,listbox->lnselcolor,listbox->font,listx->name,1);//显示名字
						}
						listbox->sta=0;	  
					}
				}
