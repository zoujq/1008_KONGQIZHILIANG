...
//height:高度必须为:FLBOX_ITEM_HEIGHT*N+FLBOX_PATH_HEIGHT+FLBOX_FINFO_HEIGHT,这里默认为40+20的N倍 
//width:宽度必须大于等于FLBOX_ITEM_HEIGHT(20)
...
_filelistbox_obj * filelistbox_creat(u16 left,u16 top,u16 width,u16 height,u8 type,u8 font)
{
	...
	if(height<=FLBOX_PATH_HEIGHT+FLBOX_ITEM_HEIGHT+FLBOX_FINFO_HEIGHT)return NULL;//高度小了	    
	if((height-FLBOX_PATH_HEIGHT-FLBOX_FINFO_HEIGHT)%FLBOX_ITEM_HEIGHT)return NULL;//尺寸不合格 	 				    
	if(width<FLBOX_ITEM_HEIGHT)return NULL;			//尺寸不合格
	...
	scb_crt=scrollbar_creat(left+width-FLBOX_SCB_WIDTH,top+FLBOX_PATH_HEIGHT,FLBOX_SCB_WIDTH,height-FLBOX_PATH_HEIGHT-FLBOX_FINFO_HEIGHT,0x80);//创建scrollbar.
	if(scb_crt==NULL)
	{
		gui_memin_free(filelistbox_crt);//释放之前申请的内存
		return NULL;					//内存分配不够.
 	}
 	scb_crt->itemsperpage=(height-FLBOX_PATH_HEIGHT-FLBOX_FINFO_HEIGHT)/FLBOX_ITEM_HEIGHT;	//每页显示的条目数
	...
	switch(key->intype)
	{
		case IN_TYPE_TOUCH:	//触摸屏按下了
 			if(filelistbox!=NULL)//listbox非空
			{
				...
				if((filelistbox->top+FLBOX_PATH_HEIGHT)<=key->y&&key->y<=(filelistbox->top+filelistbox->height-FLBOX_FINFO_HEIGHT)&&filelistbox->left<=key->x&&key->x<(endx))//在items区域内
				{ 
					...
					}else if(filelistbox->sta&(1<<6))//单点按下
					{
						...
						{	  
							if((filelistbox->selindex<(filelistbox->scbv->topitem+filelistbox->scbv->itemsperpage))&&(filelistbox->selindex>=filelistbox->scbv->topitem))//老的selindex在屏幕范围内
							{
								...
								gui_fill_rectangle(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT,endx-filelistbox->left+1,FLBOX_ITEM_HEIGHT,filelistbox->lbkcolor);//恢复底色
 								gui_draw_icos(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-FLBOX_ICO_HEIGHT)/2,filelistx->type);//画ICO图标																				    
								gui_show_ptstr(filelistbox->left+FLBOX_ICO_WIDTH+2,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-filelistbox->font)/2,endx,GUI_LCD_H,0,filelistbox->lncolor,filelistbox->font,filelistx->name,1);//显示名字
							}
							filelistbox->selindex=filelistbox->scbv->topitem+tempindex; 
							filelistx=filelist_search(filelistbox->list,filelistbox->selindex);//得到listindex的名字
							gui_fill_rectangle(filelistbox->left+FLBOX_ICO_WIDTH,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT,endx-filelistbox->left+1-FLBOX_ICO_WIDTH,FLBOX_ITEM_HEIGHT,filelistbox->lnselbkcolor);//填充选中后的底色
							gui_draw_icos(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-FLBOX_ICO_HEIGHT)/2,filelistx->type);//画ICO图标																				    
							gui_show_ptstr(filelistbox->left+FLBOX_ICO_WIDTH+2,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-filelistbox->font)/2,endx,GUI_LCD_H,0,filelistbox->lnselcolor,filelistbox->font,filelistx->name,1);//显示名字
					...
					}else //空闲状态,既没有滑动,也没单点按,执行走字显示.
					{
						...
						{	    
							if(filelistbox->selindex>=filelistbox->scbv->topitem&&filelistbox->selindex<(filelistbox->scbv->topitem+filelistbox->scbv->itemsperpage))//选中条目在当前的显示范围内
	 						{	 
								i=endx-filelistbox->left-FLBOX_ICO_WIDTH-2;//得到显示文字区域的长度
								if(i<filelistbox->namelen)//小于文字长度,就是不能够一次显示的
								{
 		 							tempindex=filelistbox->selindex-filelistbox->scbv->topitem;
									gui_fill_rectangle(filelistbox->left+FLBOX_ICO_WIDTH,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT,endx-filelistbox->left+1-FLBOX_ICO_WIDTH,FLBOX_ITEM_HEIGHT,filelistbox->lnselbkcolor);//填充选中后的底色
		 							gui_show_ptstr(filelistbox->left+FLBOX_ICO_WIDTH+2,filelistbox->top+FLBOX_PATH_HEIGHT+tempindex*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-filelistbox->font)/2,endx,GUI_LCD_H,filelistbox->curnamepos,filelistbox->lnselcolor,filelistbox->font,filelistbox->fname,1);//显示名字
...
void filelistbox_draw_list(_filelistbox_obj *filelistbox)
{
	...
	if(filelistbox->scbv->totalitems>filelistbox->scbv->itemsperpage)
	{
 	...
 	gui_fill_rectangle(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT,endx-filelistbox->left+1,filelistbox->height-FLBOX_PATH_HEIGHT-FLBOX_FINFO_HEIGHT,filelistbox->lbkcolor);//清空为底色.	
	...
	{	
		...
		{
			gui_fill_rectangle(filelistbox->left+FLBOX_ICO_WIDTH,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT,endx-filelistbox->left+1-FLBOX_ICO_WIDTH,FLBOX_ITEM_HEIGHT,filelistbox->lnselbkcolor);//填充底色
		...
		gui_draw_icos(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-FLBOX_ICO_HEIGHT)/2,filelistx->type);//画ICO图标																				    
		gui_show_ptstr(filelistbox->left+FLBOX_ICO_WIDTH+2,filelistbox->top+FLBOX_PATH_HEIGHT+i*FLBOX_ITEM_HEIGHT+(FLBOX_ITEM_HEIGHT-filelistbox->font)/2,endx,GUI_LCD_H,0,tempcolor,filelistbox->font,filelistx->name,1);//显示名字
...
void filelistbox_show_info(_filelistbox_obj *filelistbox)
{
	...
	FATFS *fs1;
	u8 res;
    DWORD freclust=0,fresec=0,totsec=0;
 	u8 * temp;
	u8 * temp1;
	u8 * mark;
	...
	gui_fill_rectangle(filelistbox->left,filelistbox->top+filelistbox->height-FLBOX_FINFO_HEIGHT,filelistbox->width,FLBOX_FINFO_HEIGHT,filelistbox->pthinfbkcolor);//显示文件信息背景色
	gui_draw_hline(filelistbox->left,filelistbox->top+filelistbox->height-FLBOX_FINFO_HEIGHT,filelistbox->width,0X000);//显示黑色的横线		
	filelistx=filelist_search(filelistbox->list,filelistbox->selindex);//得到选中的list的详细信息
	if(filelistx->type==FICO_DISK)//当前选中的是磁盘
	{
		if(filelistbox->selindex==0)mark="0:";//磁盘0
		else mark="1:";//磁盘1
		res=f_getfree((const TCHAR*)mark,&freclust,&fs1);//获得剩余容量
	    if(res==0)
		{											   
	...
	gui_show_ptstr(filelistbox->left,filelistbox->top+filelistbox->height-FLBOX_FINFO_HEIGHT+(FLBOX_FINFO_HEIGHT-FLBOX_PTHINF_FONT)/2,filelistbox->width+filelistbox->left,GUI_LCD_H,0,filelistbox->lncolor,FLBOX_PTHINF_FONT,temp,1);//显示路径
	gui_memin_free(temp);
...
{															  
	...
	gui_fill_rectangle(filelistbox->left,filelistbox->top,filelistbox->width,FLBOX_PATH_HEIGHT-1,filelistbox->pthinfbkcolor);//显示路径背景色
	gui_draw_hline(filelistbox->left,filelistbox->top+FLBOX_PATH_HEIGHT-1,filelistbox->width,0X000);//显示黑色的横线		
...
u8 filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
	...
	}else//没有任何有效路径,说明回到选择磁盘的状态了
	{
		...
		filelistbox->foldercnt=0;
		filelistbox->filecnt=0;
 		filelistbox_addlist(filelistbox,(u8*)FLIST_DISK_NAME_TBL[0][gui_phy.language],0);		//磁盘0
		filelistbox_addlist(filelistbox,(u8*)FLIST_DISK_NAME_TBL[1][gui_phy.language],0);		//磁盘1
 	}
...
void filelistbox_check_filelist(_filelistbox_obj *filelistbox)
{
	...
	{   
		...
		{	
			if(filelistbox->selindex==0)str="0:";//选中磁盘0,SD卡.
		   	else str="1:";//磁盘1,flash盘
