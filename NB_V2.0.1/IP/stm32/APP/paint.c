...
//颜色表路径
const u8 *PAINT_COLOR_TBL_PIC="0:/SYSTEM/APP/PAINT/COLORTBL.bmp";
#else
//颜色表路径
const u8 *PAINT_COLOR_TBL_PIC="1:/SYSTEM/APP/PAINT/COLORTBL.bmp";
#endif
...
u8 paint_pen_color_set(u16 x,u16 y,u16* color,u8*caption)
{
	...
	else
	{
		...
		while(rval==0)
		{
			...
			{
				tempcolor=gui_phy.read_point(tp_dev.x,tp_dev.y);	
				cmask=1;
			}	
 		}
	}
...
u8 paint_play(void)
{
	...
	u8 editmask=0;					//改动标志位
  	...
  	br=0;
	//先选择模式
	res=app_items_sel(30,64,180,72+40*2,(u8**)paint_mode_tbl[gui_phy.language],2,(u8*)&br,0X90,(u8*)APP_MODESEL_CAPTION_TBL[gui_phy.language]);//2个选择
	...
	{	  
 		   	...
 		   	while(rval==0)
			{
				...
				{	    
					...
					if(pname==NULL)rval=1;//申请失败
					{
						...
						LCD_Clear(WHITE);
						ai_load_picfile(pname,0,0,240,320);				//解码这个图片
    					break;
					} 
				}
			}
		...
		{		 
			...
			if(key==4)						//WK_UP按下
			{
				app_read_bkcolor(30,40,180,220,pset_bkctbl);//读取背景色
				//弹出对话框	 																				   
				res=app_items_sel(30,60,180,72+40*2,(u8**)pen_set_tbl[gui_phy.language],2,(u8*)&br,0X90,(u8*)paint_penset_tbl[gui_phy.language]);//2个选择
				if(res==0)//确认键按下
				{
					if(br==0)	//设置画笔颜色
					{
						paint_pen_color_set(30,40,&pencolor,(u8*)pen_set_tbl[gui_phy.language][0]);
					}else 		//设置画笔尺寸
					{
						paint_pen_size_set(30,60,pencolor,&mode,(u8*)pen_set_tbl[gui_phy.language][1]);
					}
				}
				app_recover_bkcolor(30,40,180,220,pset_bkctbl);//恢复背景色
 			} 
			...
			{	
				...
				{
					editmask=1;
	 				paint_draw_point(tp_dev.x,tp_dev.y,pencolor,mode);	//画图	     			   
				}
			}else									//没有按键按下的时候 
			{
				delay_ms(1000/OS_TICKS_PER_SEC);	//延时一个时钟节拍	   
	...
	{	  
 		app_read_bkcolor(20,120,200,80,pset_bkctbl);	//读取背景色
 		res=window_msg_box(20,120,200,80,"",(u8*)APP_SAVE_CAPTION_TBL[gui_phy.language],12,0,0X03);
		app_recover_bkcolor(20,120,200,80,pset_bkctbl);	//恢复背景色
		if(res==1)//需要保存
		{
