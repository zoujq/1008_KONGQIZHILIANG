...
#define TOMCAT_STOP_LEVEL		4			//停止阀值
#define TOMCAT_START_LEVEL		7			//启动阀值

 			  
#if ICOS_LOCATION==ICOS_SD_CARD//存放在SD卡  
const u8* TOMCAT_DEMO_PIC="0:/SYSTEM/APP/TOMCAT/Demo.bmp";//demo图片路径 	      
#else//存放在 FLASH DISK
const u8* TOMCAT_DEMO_PIC="1:/SYSTEM/APP/TOMCAT/Demo.bmp";//demo图片路径 	      
#endif

//標題
const u8* tomcat_caption_tbl[GUI_LANGUAGE_NUM]={"会说话的猫","會說話的貓","Talking TOM",};
const u8* tomcat_set_tbl[GUI_LANGUAGE_NUM]={"增益及语速设置","增益及語速設置","GAIN AND SPEED SET"};	      

//加载tom cat主界面UI	  
void tomcat_load_ui(void)
{	
 	gui_fill_rectangle(0,0,240,20,TOMCAT_TITLE_BKCOLOR);				//填充背景色
	gui_show_strmid(0,0,240,20,TOMCAT_TITLE_COLOR,16,(u8*)tomcat_caption_tbl[gui_phy.language]);//显示标题
 	gui_fill_rectangle(0,20,240,300,TOMCAT_MAIN_BKCOLOR);				//填充底色 
 	minibmp_decode((u8*)TOMCAT_DEMO_PIC,70,100,100,100,0,0);			//解码100*100的图片DEMO
...
u8 tomcat_agcspd_set(u16 x,u16 y,u8 *agc,u16 *speed,u8*caption)
{
	...
	else
	{
		...
		recoder_show_agc(x+10+24,y+38,tempagc+1);
 		gui_show_string("SPEED:     Khz",x+10,y+38+35,84,12,12,TOMCAT_INWIN_FONT_COLOR);//显示SIZE
...
{
	Audiosel_Set(3);					//切换到无声模式
	recoder_enter_rec_mode(1024*agc);				
...
void tomcat_play_wav(u8 *buf,u32 len)
{
	...
	VS_Set_Vol(250);	//音量调到最大
	Audiosel_Set(0);	//切换到1053音频输出
	do//主播放循环
    {  	
...
u8 tomcat_play(void)
{
	...
	{    
   	   	...
   	   	while(rval==0)
		{
			...
			{   
				app_read_bkcolor(30,95,180,158,(u16*)databuf);//读取背景色
				res=tomcat_agcspd_set(30,95,&recagc,&wavspd,(u8*)tomcat_set_tbl[gui_phy.language]);//设置增益和语速
				if(res==0)
				{
					VS_WR_Cmd(SPI_AICTRL1,1024*recagc);	//设置增益:1~10
					wavhead->fmt.SampleRate=(wavspd+8)*500;	//设置采样率,以获得不同的效果.
 				}	
				app_recover_bkcolor(30,95,180,158,(u16*)databuf);//恢复背景色  				  
