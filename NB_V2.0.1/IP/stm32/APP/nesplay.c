		...
		if(flistbox->dbclick==0X81)//双击文件了
		{
			...
			if(pname==NULL)rval=1;//申请失败
			{
				pname=gui_path_name(pname,flistbox->path,fn);	//文件名加入路径  
 				LCD_Clear(0X0);//黑屏 
 				FSMC_Bank1E->BWTR[6]|=2<<0; 			//地址建立时间（ADDSET）为3个HCLK  	 	 
 				FSMC_Bank1E->BWTR[6]|=3<<8; 		 	//数据保持时间（ADDSET）为4个HCLK  	 	 
 				//SRAM设置
				FSMC_Bank1->BTCR[5]|=6<<8; 				//数据保持时间（DATAST）为7个HCLK 7/128M=55ns	 	 

 				load_nes(pname);	//开始nes游戏
				LCD_Clear(BLACK);	//清屏
				FSMC_Bank1E->BWTR[6]&=0XFFFF00F0;		//清除之前的设置
				FSMC_Bank1E->BWTR[6]|=0<<0; 			//地址建立时间（ADDSET）为1个HCLK 	 	 
 				FSMC_Bank1E->BWTR[6]|=2<<8;  			//数据保持时间（ADDSET）为3个HCLK 1/36M=27ns	 	 
 				//SRAM设置复原
				FSMC_Bank1->BTCR[5]&=0XFFFF00FF;		//清除之前的设置
				FSMC_Bank1->BTCR[5]|=3<<8; 				//数据保持时间（DATAST）为4个HCLK 4/72M=55ns
 			}	       
