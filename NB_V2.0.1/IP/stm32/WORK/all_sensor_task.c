...
void Uart3_Decode(void) {
	...
	if (uUart3_Recive_Data[0] == 0xFE && uUart3_Recive_Data[1] == 0x04) {
...
void User_All_Sensor_Task(void *pdata) { //所有传感器检测任务
	...
	while (1) {
		...
		if (uTest_Mode_Time == 0) {
			...
			if (CMachine.uLine_Work == 1) { //连网状态下
...
//	 User_Test_Data_Check();//数据较准（开机时）
			...
			if (calendar.hour == 9 && calendar.min == 0 && CMachine.uTVOC_Res_Start_Time_No == 0)	CMachine.uTVOC_Res_Start_Time_No = 180; //重启
