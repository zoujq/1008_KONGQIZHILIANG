...
void NVIC_Configurationa(void)
{
	...
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_1);	//配置优先级为组1（抢占式优先级1位，副优先级有三位）

	/* Enable the RTC Interrupt */
	//配置RTC中断
	NVIC_InitStructure.NVIC_IRQChannel = RTC_IRQn; //中断通道为RTC全局通道
...
//u32 tempCountValue;
 ...
 NVIC_Configurationa();
	...
	//	RTC_Set(2009,12,2,10,0,55);  //设置时间	
			...
			//	RTC_WaitForLastTask();
		//	RTC_SetAlarm(RTC_GetCounter() + 5);	//设置新的闹钟值
		//	RTC_WaitForLastTask();
...
void RTCAlarm_IRQHandler(void)
{
	...
	//	RCC_Configuration();
//	RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR | RCC_APB1Periph_BKP, ENABLE);//待机唤醒，需重新使能
