...
char User_Key_Scan_Thread(thread_t* pt) {
	...
	Thread_BEGIN {
		...
		Sys_Cntrl.uSys_User_Reset = 0; //系统用户重置
		Sys_Cntrl.uSys_User_Reset_Time = 0; //系统用户重置,计时
		Sys_Cntrl.uSys_User_Reset_Time_LED_Blink_No = 0; //系统用户重置,计时闪烁
		...
		while (f_Sys_Check == 2) {
			...
			if (KeyPress == 0x00) {
				...
				if (fKey0_Short == 0 && CMachine.UI_Show_No == 1 && (CMachine.fCharge == 0)) {
					printf("SYS_POWER_OFF_1\n\r");
					SYS_POWER_OFF(); //系统关闭
				}
				...
				if (KeyPress == 0x07) { //按键4//开关机和模式组合键
					if (fKey3_Short != 2) Key3_No++;
					if (Key3_No == (KEY_DELAY - 1)) {
						fKey3_Short = 1; //按键4短按

						if (CMachine.UI_Show_No > 1 && uTime_Set_Mode == 0) {	//非时间设置模式下
							//if(CMachine.UI_Show_No==7)	//显示二维码
							//{
							CMachine.uID_Show_Time_No = 1000; //10秒
							CMachine.uID_Show_Time_No_B = 1; //

//					CMachine.fSys_User_Other_Data=1;//上传一次数据
							Sys_Cntrl.uSys_User_Reset = 1; //系统用户重置
							Sys_Cntrl.uSys_User_Reset_Time = 12000; //系统用户重置,计时
							//	Sys_Cntrl.uSys_User_Reset_Time_LED_Blink=300;//系统用户重置,计时闪烁，3秒
