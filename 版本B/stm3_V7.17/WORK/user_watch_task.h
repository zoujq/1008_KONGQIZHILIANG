...
typedef struct
{
	...
	unsigned short uID_Show_Time_No;//显示ID号
	unsigned short uID_Show_Time_No_B;//显示ID号

		
	unsigned char uLanguage_Mode;//进入较准模式		
	unsigned char uLanguage_Mode_B;//进入较准模式		

	unsigned char uLOGO_Mode;//LOGO		
	unsigned char uLOGO_Mode_B;//LOGO
	unsigned char uLOGO_Mode_Clear;	


  unsigned char fCo2_b03_Clear;
  unsigned char fHcho_b03_Clear;
...
//	unsigned short uSys_User_Reset_Time_LED_Blink_No;//系统用户重置,LED闪烁显示	
	...
	//unsigned char fSys_User_Other_Data;//其他信息上传			
		
		
//	unsigned char	uLine_Work_B;//联网状态		
