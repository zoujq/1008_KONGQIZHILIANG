...
_t9_obj * t9_creat(u16 left,u16 top,u8 mode)
{
	...
	for(i=0;i<8;i++)
	{
		t9_crt->hbtn[i]=btn_creat(left+30*i+1,top+1,29-temp,20,i+12,0X04);//文字按钮,背景色单一
 	...
 	for(i=0;i<5;i++)
	{
		t9_crt->vbtn[i]=btn_creat(left+1,top+i*22+23,44,21,i+20,0X04);//文字按钮,背景色单一
...
void t9_ncr_clr(_t9_obj * t9x)
{
 	...
 	t9x->ncr_status=0;//结束一次识别
  	t9x->ncr_timer_old=0;		 
	gui_fill_rectangle(t9x->left+NCR_LEFT_OFFSET,t9x->top+NCR_TOP_OFFSET,NCR_WIDTH,NCR_HEIGHT,0XFFFF);//填充白色
}
 	...
 	switch(key->intype)
	{
		case IN_TYPE_TOUCH:	//触摸屏按下了
			...
			{	
				...
				if((t9x->top+NCR_TOP_OFFSET)<key->y&&key->y<(t9x->top+NCR_TOP_OFFSET+NCR_HEIGHT-1)&&(t9x->left+NCR_LEFT_OFFSET)<key->x&&key->x<(t9x->left+NCR_LEFT_OFFSET+NCR_WIDTH-1))//在手写框内部
				{	    
					t9x->ncr_status=0X80;//有按键按下	 
					gui_draw_bigpoint(key->x,key->y,RED);//红色画点
 					if(t9x->ncr_tcnt<200)//记录最多200个坐标轨迹.
					{
						tcnt=t9x->ncr_tcnt;
						if(tcnt)tcnt--;
						if(t9x->pbuf[tcnt].x!=(key->x-t9x->left-NCR_LEFT_OFFSET)||t9x->pbuf[tcnt].y!=(key->y-t9x->top-NCR_TOP_OFFSET))//新坐标,必须和老坐标不相等,否则不予采集!!!
						{									 
...
{
	u8 cnt=0;
	if(t9x->disstr[0]==NULL)//输入字符为0,加入0X8,表示退格 
	{
...
{
	u8 cnt=0;
	if(t9x->disstr[0]!=NULL)//输入字符不为0,取输入字符串的第一个字符/汉字 
	{
...
void t9_test_task(void)
{
	...
	tedit=edit_creat(10,20,200,23,0,2,16);//创建按钮
	...
	f_txt=(FIL *)mymalloc(SRAMIN,sizeof(FIL));	//开辟FIL字节的内存区域 
 	...
 	res=f_open(f_txt,(const TCHAR*)"0:/test.txt",FA_READ|FA_WRITE);//打开文件
	if(res==0)
	{
		tmemo=memo_creat(10,45,200,105,0,2,16,f_txt->fsize+100);//申请整个txt这么多的内存	
	...
	{		 
		...
		if(t9->outstr[0]!=NULL)//打印输入的字符
		{
			printf("%s\r\n",t9->outstr);
			edit_add_text(tedit,t9->outstr);
			memo_add_text(tmemo,t9->outstr);
			if(strcmp((const char *)t9->outstr,"退")==0)i=0;//输入退字,则退出
		    t9->outstr[0]=NULL;
