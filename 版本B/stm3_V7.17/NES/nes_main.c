...
{
	ram6502=(u8*)mymalloc(SRAMIN,2048);	//申请2K内存
	if(ram6502==NULL)return 1;			//申请失败
	NameTable=(u8*)mymalloc(SRAMIN,2048);//申请2K内存
	if(NameTable==NULL)return 2;
	Buffer_scanline=(u16*)mymalloc(SRAMIN,(8+256+8)*2);
...
{
	myfree(SRAMIN,ram6502);
	myfree(SRAMIN,NameTable);
	myfree(SRAMIN,Buffer_scanline);
} 		
