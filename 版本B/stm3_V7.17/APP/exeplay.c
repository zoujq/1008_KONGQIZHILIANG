...
//执行BIN文件前的提示
const u8* exeplay_remindmsg_tbl[GUI_LANGUAGE_NUM]=
{
...
//警告
const u8* exeplay_warning_tbl[GUI_LANGUAGE_NUM]=
{
"警告:",
"警告:",
"Warning:",
...
//APP程序非法提示语
const u8* exeplay_apperrmsg_tbl[GUI_LANGUAGE_NUM]=
{					   							    
...
u8 exe_play(void)
{
	...
	app_filebrower((u8*)APP_MFUNS_CAPTION_TBL[9][gui_phy.language],0X07);//显示信息
		...
		{	    
			...
			if(pname==NULL)rval=1;//申请失败
			{
				...
				{  
					...
					{
	  					res=window_msg_box(20,80,200,160,(u8*)exeplay_remindmsg_tbl[gui_phy.language],(u8*)exeplay_warning_tbl[gui_phy.language],16,0,0X03);//显示提醒信息
						if(res==1)//按下了确认键
						{
					...
					{
						window_msg_box(40,120,160,80,(u8*)exeplay_apperrmsg_tbl[gui_phy.language],(u8*)exeplay_warning_tbl[gui_phy.language],12,0,0);//提示非法的APP文件
