...
const u8* appplay_appname_tbl[GUI_LANGUAGE_NUM][16]=
{
{
	"红外遥控","应用程序","应用程序","应用程序",
	"应用程序","应用程序","应用程序","应用程序",
...
//appplay的图标路径表
const u8 * appplay_icospath_tbl[16]=
{
"0:/SYSTEM/APP/APPS/ICOS/remote.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app01.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app02.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app03.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app04.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app05.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app06.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app07.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app08.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app09.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app10.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app11.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app12.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app13.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app14.bmp",
"0:/SYSTEM/APP/APPS/ICOS/app15.bmp",
};
#else//存放在FLASH DISK
//appplay的图标路径表
const u8 * appplay_icospath_tbl[16]=
...
///////////////////////////////////////////////////////////////////////////////////
//红外遥控条目信息列表
const u8* appplay_remote_items_tbl[GUI_LANGUAGE_NUM][3]=
{
{"  键值:","  次数:","  符号:",},
{"  鍵值:","  次數:","  符號:",},
...
u8 appplay_load_ui(m_app_dev *appdev) 
{
	...
	if(twin)
	{
		...
		gui_fill_rectangle(0,0,240,320,APPPLAY_EX_BACKCOLOR);//填充背景色
	  	app_gui_tcbar(0,0,240,20,0x02);			//下分界线	 
		gui_show_strmid(0,0,240,20,WHITE,16,(u8*)appplay_caption_tbl[gui_phy.language]);//显示标题  
		window_draw(twin);
 		for(i=0;i<16;i++)
		{			 
...
u8 appplay_tpscan(m_app_dev *appdev)
{
	...
	if((tp_dev.sta&TP_PRES_DOWN))//有按键被按下
	{
		if(firsttpd==0)//第一次处理?
		{
			...
			for(i=0;i<16;i++)
			{
				if((tp_dev.x>appdev->icos[i].x)&&(tp_dev.x<appdev->icos[i].x+appdev->icos[i].width)
				 &&(tp_dev.y>appdev->icos[i].y)&&(tp_dev.y<appdev->icos[i].y+appdev->icos[i].height))//在区域内
				{
					break;//得到选中的编号	
				}
			}
...
u8 Remote_Play(u8* caption)	 
{
	...
	u8 key;		 
 	_window_obj* twin=0;	//窗体
   	twin=window_creat(60,105,120,100,0,1<<6|1<<0,16);//创建窗口
	if(twin)
	{	
		twin->caption=caption;					 
	 	twin->windowbkc=APP_WIN_BACK_COLOR;	 								//窗体主色
		window_draw(twin);													//画窗体
	 	app_draw_smooth_line(60+5,105+32+1+22,110,1,0Xb1ffc4,0X1600b1);		//画彩线
	 	app_draw_smooth_line(60+5,105+32+1+22+22,110,1,0Xb1ffc4,0X1600b1);	//画彩线
		BACK_COLOR=APP_WIN_BACK_COLOR;
 		//显示条目信息
		gui_show_ptstr(60+8,105+32+1+3,60+8+56,105+32+1+3+16,0,BLACK,16,(u8*)appplay_remote_items_tbl[gui_phy.language][0],1);
		gui_show_ptstr(60+8,105+32+1+3+22,60+8+56,105+32+1+3+22+16,0,BLACK,16,(u8*)appplay_remote_items_tbl[gui_phy.language][1],1);
		gui_show_ptstr(60+8,105+32+1+3+44,60+8+56,105+32+1+3+44+16,0,BLACK,16,(u8*)appplay_remote_items_tbl[gui_phy.language][2],1);
 	}else rval=1;
	if(rval==0)
	{
		...
		{	  
			...
			if(key)
			{				 	 			   
				LCD_ShowNum(60+8+56,105+32+1+3,key,3,16);		//显示键值
				LCD_ShowNum(60+8+56,105+32+1+3+22,RmtCnt,3,16);	//显示按键次数
 				switch(key)
				{			    
					case 162: 
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"POWER");	  
						break;
					case 98:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"UP    ");
						break;
					case 2:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"PLAY  ");
						break;
					case 226:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"ALIEN ");
						break;
					case 194:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"RIGHT ");
						break;
					case 34:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"LEFT  ");
						break;
					case 56:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"8     ");
						break;
					case 224:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"VOL-  ");
						break;
					case 168:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"DOWN  ");
						break;
					case 144:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"VOL+  ");
						break;
					case 104:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"1     ");
						break;
					case 152:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"2     ");
						break;
					case 176:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"3     ");
						break;
					case 48:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"4     ");
						break;
					case 24:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"5     ");
						break;
					case 122:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"6     ");
						break;
					case 16:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"7     ");
						break;
					case 90:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"9     ");
						break;
					case 66:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"0     ");
						break;
					case 82:
						LCD_ShowString(60+8+56,105+32+1+3+44,240,320,16,"DELETE");
						break;	    
				}
			} 
			delay_ms(10); 
