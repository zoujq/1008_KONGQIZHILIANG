...
#define RECODER_TITLE_COLOR   	0XFFFF		//录音机标题颜色	
#define RECODER_TITLE_BKCOLOR	0X0000		//录音机标题背景色	

#define RECODER_VU_BKCOLOR    	0X39C7		//VU Meter背景色	
#define RECODER_VU_L1COLOR    	0X07FF		//VU Meter L1色	
#define RECODER_VU_L2COLOR    	0xFFE0		//VU Meter L2色	
#define RECODER_VU_L3COLOR    	0xF800		//VU Meter L3色	

#define RECODER_TIME_COLOR    	0X07FF		//时间颜色
#define RECODER_MAIN_BKCOLOR	0X18E3		//主背景色

//窗体内嵌字颜色
#define RECODER_INWIN_FONT_COLOR		0X736C		//0XAD53		

#if ICOS_LOCATION==ICOS_SD_CARD//存放在SD卡   
const u8* RECODER_DEMO_PIC="0:/SYSTEM/APP/RECODER/Demo.bmp";		//demo图片路径 	      
const u8* RECODER_RECR_PIC="0:/SYSTEM/APP/RECODER/RecR.bmp";		//录音 松开
const u8* RECODER_RECP_PIC="0:/SYSTEM/APP/RECODER/RecP.bmp";		//录音 按下
const u8* RECODER_PAUSER_PIC="0:/SYSTEM/APP/RECODER/PauseR.bmp";	//暂停 松开
const u8* RECODER_PAUSEP_PIC="0:/SYSTEM/APP/RECODER/PauseP.bmp";	//暂停 按下
const u8* RECODER_STOPR_PIC="0:/SYSTEM/APP/RECODER/StopR.bmp";		//停止 松开
const u8* RECODER_STOPP_PIC="0:/SYSTEM/APP/RECODER/StopP.bmp";		//停止 按下
#else//存放在 FLASH DISK
const u8* RECODER_DEMO_PIC="1:/SYSTEM/APP/RECODER/Demo.bmp";		//demo图片路径 	      
const u8* RECODER_RECR_PIC="1:/SYSTEM/APP/RECODER/RecR.bmp";		//录音 松开
const u8* RECODER_RECP_PIC="1:/SYSTEM/APP/RECODER/RecP.bmp";		//录音 按下
const u8* RECODER_PAUSER_PIC="1:/SYSTEM/APP/RECODER/PauseR.bmp";	//暂停 松开
const u8* RECODER_PAUSEP_PIC="1:/SYSTEM/APP/RECODER/PauseP.bmp";	//暂停 按下
const u8* RECODER_STOPR_PIC="1:/SYSTEM/APP/RECODER/StopR.bmp";		//停止 松开
const u8* RECODER_STOPP_PIC="1:/SYSTEM/APP/RECODER/StopP.bmp";		//停止 按下
#endif 
...
void recoder_wav_init(__WaveHeader* wavhead) //初始化WAV头			   
{
	...
	wavhead->fmt.AudioFormat=0X01; 		//0X01,表示PCM;0X01,表示IMA ADPCM
 	...
 	wavhead->fmt.SampleRate=8000;		//16Khz采样率 采样速率
...
void recoder_vu_meter(u16 x,u16 y,u8 level)
{
	...
	if(level==0)
	{
		gui_fill_rectangle(x,y,218,10,RECODER_VU_BKCOLOR);	//填充背景色
		return;
	}   
	for(i=0;i<level;i++)
	{
	...
	if(level<10)gui_fill_rectangle(x+level*22,y,218-level*22,10,RECODER_VU_BKCOLOR);	//填充背景色	 
}   
//显示录音时长
//显示尺寸为:150*60
//x,y:地址
...
{
 	gui_fill_rectangle(x-1,y-1,240,12,RECODER_MAIN_BKCOLOR);	//填充背景色
 	gui_show_ptstrwhiterim(x,y,240,y+12,0,BLACK,WHITE,12,name); 	  
}
//加载录音机主界面UI	  
void recoder_load_ui(void)
{	
 	gui_fill_rectangle(0,0,240,20,RECODER_TITLE_BKCOLOR);				//填充背景色
	gui_show_strmid(0,0,240,20,RECODER_TITLE_COLOR,16,(u8*)APP_MFUNS_CAPTION_TBL[13][gui_phy.language]);//显示标题
 	gui_fill_rectangle(0,20,240,300,RECODER_MAIN_BKCOLOR);				//填充底色 
 	minibmp_decode((u8*)RECODER_DEMO_PIC,70,100,100,100,0,0);			//解码100*100的图片DEMO
	recoder_vu_meter(11,200+5,0);										//显示vu meter;
...
void recoder_show_agc(u16 x,u16 y,u8 agc)
{
	...
	if(agc==0)
	{
   		gui_show_string("AUTO",x,y,24,12,12,RECODER_INWIN_FONT_COLOR);//显示agc
	}else
	{
		gui_show_num(x,y,2,RECODER_INWIN_FONT_COLOR,12,agc,0X80);	//显示数值	
	}
}	
//agc设置界面.固定尺寸:180*122    
//x,y:左上角坐标
//agc:自动增益指针(0~15)0,自动增益;15,15倍增益
//caption:窗口名字				  
//返回值:0,成功设置;
//    其他,不进行设置
u8 recoder_agc_set(u16 x,u16 y,u8 *agc,u8*caption)
{
	...
	else
	{
		...
		progressbar_draw_progressbar(agcprgb);
   		gui_show_string("AGC:",x+10,y+38,24,12,12,RECODER_INWIN_FONT_COLOR);//显示SIZE
 		...
 		while(rval==0)
		{
 			...
 			if(res&&(tempagc!=agcprgb->curpos))	//进度条改动了
			{
				tempagc=agcprgb->curpos;		//保存最新的结果  
 				recoder_show_agc(x+10+24,y+38,tempagc);
				if(tempagc<64)VS_WR_Cmd(SPI_AICTRL1,1024*tempagc);//设置增益,0,自动增益.1024相当于1倍,512相当于0.5倍,最大值65535=64倍	
   				else VS_WR_Cmd(SPI_AICTRL1,65535);	//设置到最大增益
			} 	
 		}
	}
...
//本部分加载字体
	...
	{
		res=f_open(f_rec,(const TCHAR*)APP_ASCII_60,FA_READ);//打开文件夹
		if(res==FR_OK)
		{
			bfbase=(u8*)gui_memex_malloc(f_rec->fsize);	//为大字体开辟缓存地址
			if(bfbase==0)rval=1;
			else 
			{
				res=f_read(f_rec,bfbase,f_rec->fsize,(UINT*)&br);	//一次读取整个文件
 			}
		} 
		if(res)rval=1;
	} 	    
/////////////////////////////////////////////////////////////////////////////////
	...
	mbtn=btn_creat(0,300,40,19,0,0x03);		//创建文字按钮
	if(mbtn==NULL)rval=1;					//没有足够内存够分配
	else
	{																				
	...
	recbtn=btn_creat(48,215+18,48,48,0,1);		//创建图片按钮
	if(recbtn==NULL){rval=1;}				//创建失败.
	else
	{
	...
	stopbtn=btn_creat(48+96,215+18,48,48,0,1);	//创建图片按钮
	if(stopbtn==NULL){rval=1;}				//创建失败.
	else
	{
	...
	pname=gui_memin_malloc(40);	//申请40个字节内存,类似"0:RECORDER/REC20120321210633.wav"
	if(pname==NULL)rval=1;
	pset_bkctbl=gui_memex_malloc(180*122*2);//为设置时的背景色表申请内存
	...
	{    
		...
		btn_draw(stopbtn);
 		if(mp3dev!=NULL)mp3_stop(mp3dev);	//先停止MP3播放
  		Audiosel_Set(0);					//设置为音频输出为录音机输出音频
		recoder_enter_rec_mode(1024*recagc);				
   		while(VS_RD_Reg(SPI_HDAT1)>>8);		//等到buf 较为空闲再开始  
  		recoder_show_time(45,40,bfbase,recsec);//显示时间
 	   	while(rval==0)
		{
			tp_dev.scan(0);    
			in_obj.get_key(&tp_dev,IN_TYPE_TOUCH);	//得到按键键值   
			...
			{   
				app_read_bkcolor(30,95,180,122,pset_bkctbl);//读取背景色
				res=recoder_agc_set(30,95,&recagc,(u8*)recoder_set_tbl[gui_phy.language]);//设置AGC
				if(res==0)
				{
					if(recagc<64)VS_WR_Cmd(SPI_AICTRL1,1024*recagc);//设置增益,0,自动增益.1024相当于1倍,512相当于0.5倍,最大值65535=64倍	
	   				else VS_WR_Cmd(SPI_AICTRL1,65535);				//设置到最大增益
				}	
				app_recover_bkcolor(30,95,180,122,pset_bkctbl);//恢复背景色  				  
			}
			...
			{  
				...
				}else	//还没开始录音 
				{
					...
					recoder_show_name(2,24,pname);	//显示名字
...
//读取数据			  
			...
			if((w>=256)&&(w<896))
			{
				...
				{	 
					...
					recoder_vu_meter(11,200+5,vulevel);//显示vu meter;
					maxval=0;							  
	 			}  
