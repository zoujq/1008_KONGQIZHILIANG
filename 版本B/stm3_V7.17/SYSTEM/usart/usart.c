 ...
 static u8 Res1;	
	...
	OSIntEnter();    
	 ...
	 if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾) 
		{
		 Res1 =USART_ReceiveData(UART4);//(USART1->DR);	//读取接收到的数据
...
void uart4_init(u32 bound){
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE);	//使能GPIOC\D时钟以及复用功能时钟
...
u8  fUart_Send_End=0;//发送完成 标志，数据分段发送

char  uUart2_Recive_Data[UART_RX_BUF_MAX]={0};//接收缓存
u8 	uUart2_Recive_Length=0;//接收长度
u8 	uUart2_Recive_End=0;//接收完成

char  uUart2_Send_Data[UART_TX_BUF_MAX]={0};//发送缓存
u8 	uUart2_Send_Length=0;//发送长度
u8 	uUart2_Send_Length_No=0;//发送长度,计数
...
void User_UART_Send(void)
{
	...
	USART_ClearFlag(USART2, USART_FLAG_TC);//清除传输完成标志位,否则可能会丢失第1个字节的数据.网友提供.
...
static	u8 Res;
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		uUart2_Recive_End=5;//接收完成后50ms反应
...
char Uart_10ms_Thread(thread_t* pt)//串口10ms任务
{
     Thread_BEGIN
    {
...
//		Mqtt_Progress(rxbuf,rxlen);//主循环做异步处理
		uUart2_Recive_Length=0;
				...
				if(strstr(_net.rxbuf,"ERROR")!=0)//检测到错误直接重启
				{
...
//使能USART2，GPIOA时钟
 ...
 USART_ITConfig(USART2, USART_IT_TC, ENABLE);//Tramsimssion Complete后，才产生中断. 开TC中断必须放在这里,否则还是会丢失第一字节	 
